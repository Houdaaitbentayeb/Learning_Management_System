...
public class QuizService {
    ...
    private Quiz requireQuizAndCheckAccess(int quizId, HttpServletRequest request) {
        Users loggedInUser = sessionAuthService.requireUser(request);
        ...
        Quiz quiz = quizRepository.findById(quizId)
                .orElseThrow(() -> new IllegalArgumentException("No quiz found with the given ID: " + quizId));

        boolean isInstructorOfCourse =
                courseRepository.findByInstructorId(loggedInUser.getUserId(), quiz.getCourse().getCourseId());

        // Instructor access
        if (loggedInUser.getUserTypeId() != null && loggedInUser.getUserTypeId().getUserTypeId() == 3) {
        ...
        // Other roles (admin etc.) : adapte si tu veux les autoriser
        throw new IllegalArgumentException("You don't have permission to enter this quiz.");
    }
    ...
    public List<QuestionDto> getQuizQuestions(int id, HttpServletRequest request) throws Exception {
        Users loggedInUser = sessionAuthService.requireUser(request);
        Quiz quiz = requireQuizAndCheckAccess(id, request);

        // si étudiant : règles supplémentaires (temps + resubmit)
    ...
    public int quizFeedback(int quiz_id, int student_id, HttpServletRequest request) throws Exception {
        Users loggedInUser = sessionAuthService.requireUser(request);
        Quiz quiz = requireQuizAndCheckAccess(quiz_id, request);

        // règle spécifique étudiant : pas voir notes des autres
        if (loggedInUser.getUserTypeId().getUserTypeId() == 2) {
